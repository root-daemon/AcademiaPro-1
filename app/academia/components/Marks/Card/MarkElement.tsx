...
export function MarkDisplay({ marks }: { marks: Marks & Overall }) {
	...
	return (
		<div
			className={`${Number(scored) === Number(marks.total) && !scored ? "border-light-success-color dark:border-dark-success-color" : Number(scored) < Number(marks.total) / 2 && !scored ? "border-light-error-color dark:border-dark-error-color border-dashed" : "border-transparent"} flex items-center gap-1 rounded-full border bg-light-background-dark dark:bg-dark-background-dark`}
		>
			<span
			...
			<span
				className={`${scored ? "bg-light-success-background text-light-success-color dark:bg-dark-success-background dark:text-dark-success-color" : "bg-light-color text-light-background-light dark:bg-dark-color dark:text-dark-background-darker"} rounded-full px-2 py-0.5 text-sm font-bold opacity-80`}
			>
